...
| F-03 | 状態確認 | 待機、給水中、クールダウン、最終実行、水分値を取得する |
...
| 設定 | 初期方針 | 目的 |
|---|---|---|
| `DOSE_MS` | 流量校正後に決定 | 1回分の運転時間 |
| `MAX_RUN_MS` | `min(DOSE_MS * 1.25, 180000)` | 暴走時の絶対上限 |
| `COOLDOWN_MS` | 10分 | 二重給水防止 |
| `BOOT_GUARD_MS` | 5分 | 再起動直後の誤作動防止 |

クライアントから運転秒数を自由指定させず、「標準1回分」という命令だけを公開する。
